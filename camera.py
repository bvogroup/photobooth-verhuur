...
class EDSDKWorker(QThread):
    ...
    def _do_capture(self, use_af):
        ...
        try:
            ...
            # Send capture command with retry + fallback
            # 1. Try TakePicture (works when AF succeeds)
            # 2. If AF fails (0x8D01) or not supported, fall back to NonAF
            # 3. If BUSY (0x81), wait and retry
            capture_sent = False
                ...
                try:
                    ...
                    if "0x00000081" in err_str:  # BUSY
                        ...
                        try:
                            ...
                            pass
                    elif "0x00008D01" in err_str or "0x8D01" in err_str:  # AF_NG
                        print(f"[CAMERA] AF mislukt, probeer NonAF... (poging {attempt+1})")
                        try:
                            self._edsdk.take_picture_no_af()
                            capture_sent = True
                            print(f"[CAMERA] NonAF capture verzonden")
                            break
                        except EDSDKError as e2:
                            print(f"[CAMERA] NonAF ook mislukt: {e2}")
                            _t.sleep(0.5)
                    else:
                        print(f"[CAMERA] TakePicture mislukt: {err_str}, probeer NonAF...")
                        try:
                            self._edsdk.take_picture_no_af()
                            capture_sent = True
                            print(f"[CAMERA] NonAF capture verzonden")
