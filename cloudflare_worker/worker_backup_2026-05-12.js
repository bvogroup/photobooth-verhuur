...
export default {
  ...
  async scheduled(event, env, ctx) {
    ...
    do {
      const listed = await env.PHOTOS.list({ cursor, limit: 500 });
...
async function handleGallery(env, sessionId, origin) {
  ...
  const html = buildGalleryHTML(slides, sessionId);
 ...
 */
function buildGalleryHTML(slides, sessionId) {
            ...
            fetch(src)
              ...
              .then(function(blob) {
                ...
                if (navigator.canShare && navigator.canShare({ files: [file] })) {
                  ...
                  return navigator.share({ title: label + ' - Bootharoo', text: 'Bekijk mijn foto van de photobooth! ­ƒô©', url: window.location.href });
                }
              })
              .catch(function(err) {
                if (err.name !== 'AbortError') {
                  navigator.share({ title: 'Mijn foto - Bootharoo', text: 'Bekijk mijn foto van de photobooth! ­ƒô©', url: window.location.href }).catch(function() {});
                }
              });
          });
        }
